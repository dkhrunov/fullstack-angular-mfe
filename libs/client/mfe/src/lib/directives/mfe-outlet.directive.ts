...
export class MfeOutletDirective implements OnChanges, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    @Inject(OPTIONS) private readonly _options: MfeOptions
  ) {}
  ...
  private async _createRemoteComponent<TComponent>(
    ...
    const { component, ngModuleRef } = await this._remoteComponentLoader.loadComponentWithModule<
      TComponent,
      unknown
    >(remoteComponent, this.injector, options);
